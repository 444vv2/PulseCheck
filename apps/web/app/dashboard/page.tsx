    ...
    try {
      ...
      setStatus(
        response.items.length
          ? "Дані оновлено щойно."
          : "Додайте перший endpoint для моніторингу.",
      );
    } catch (reason) {
      setStatus(
        reason instanceof Error
          ? reason.message
          : "Не вдалося завантажити монітори.",
      );
    }
  }, [request]);
  ...
  return (
    <main className="dashboard-shell">
      <aside className="sidebar">
        ...
        <nav>
          <a className="nav-item active" href="#monitors">
            ⌁ <span>Монітори</span>
          </a>
      ...
      <section className="dashboard-content">
        <header className="dashboard-header">
          <div>
            ...
            <h1>Доброго дня, {email.split("@")[0]}.</h1>
            <p>Ось що відбувається з вашими endpoint’ами.</p>
          </div>
        ...
        <section className="stats-grid">
          <article>
            <span>УСІ МОНІТОРИ</span>
            <strong>{monitors.length}</strong>
            <small>У вашому просторі</small>
          </article>
          <article>
            <span>АКТИВНІ</span>
            <strong>
              {monitors.filter((monitor) => monitor.isActive).length}
            </strong>
            <small className="positive">● Готові до перевірки</small>
          </article>
          <article>
            <span>ПАУЗА</span>
            <strong>
            ...
            <small>Можна відновити будь-коли</small>
          </article>
        </section>
        ...
        <section id="monitors" className="monitor-section">
          ...
          {!hasToken && (
            <div className="notice">
              <strong>Дашборд готовий.</strong> Увійдіть, щоби підключити його
              до API. <Link href="/login">До входу →</Link>
            </div>
          )}
          {hasToken && (
            <form className="add-monitor" onSubmit={addMonitor}>
              <input
              ...
              <button className="primary-button">Додати монітор</button>
            </form>
          )}
          <div className="monitor-list">
            {monitors.map((monitor) => (
              <article className="monitor-row" key={monitor.id}>
                ...
                <div className="monitor-main">
                  ...
                  <small>
                    Перевірка кожні {formatInterval(monitor.intervalSec)} ·{" "}
                ...
                <span className={`state ${monitor.isActive ? "state-up" : ""}`}>
                  {monitor.isActive ? "Активний" : "На паузі"}
                </span>
                <button
                  className="text-button"
                  onClick={() => void toggleMonitor(monitor)}
                >
                  {monitor.isActive ? "Пауза" : "Запустити"}
                </button>
                <button
            ...
            {hasToken && !monitors.length && (
              <div className="empty-state">
                Тут з’являться ваші монітори після додавання.
              </div>
            )}
          </div>
        </section>
